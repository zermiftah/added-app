        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-4">
          {STATS.map((stat, i) => (
            <Reveal key={i} delay={i * 80}>
              <div className={`text-center lg:text-left ${i < 3 ? "lg:border-r lg:border-black/8 lg:pr-8" : ""}`}>
                <div className="font-fraunces font-400 text-accent leading-none mb-3" style={{ fontSize: "clamp(32px, 4vw, 52px)" }}>
                  {stat.value}
                </div>
                <p className="font-inter text-stone leading-snug" style={{ fontSize: 13, fontWeight: 400 }}>
                  {stat.label}
                </p>
              </div>
            </Reveal>
