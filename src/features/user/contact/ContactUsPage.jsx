...
/* ── Testimonials ── */
const TESTIMONIALS = [
  { name: "Linda",  school: "Harvard'29",     photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Linda&backgroundColor=b6e3f4",  quote: "I could always count on them to respond within 24 hours. They made me feel so secure through the whole process." },
  { name: "Yosie",  school: "Yale'28",        photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Yosie&backgroundColor=ffd5dc",  quote: "I had a lot of interests but no common thread. AE hand-held me throughout — ensuring my application reflected who I really was." },
  { name: "Anna",   school: "UC Berkeley'29", photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Anna&backgroundColor=d1f0d1",   quote: "They helped me transform my love for film and media into a powerful application narrative. The strategy and essay guidance were key to my acceptance." },
  { name: "Lyla",   school: "Columbia'28",    photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Lyla&backgroundColor=ffecd2",   quote: "AddedEducation was my rock. What stood out was their ability to connect the dots across all my projects — links I wouldn't have noticed myself." },
  { name: "Aryan",  school: "Cornell'29",     photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Aryan&backgroundColor=c0d4f5",  quote: "With everything going on, AddedEducation helped me figure out my activities and align my major, making sure I got into the right programs that fit my needs and dreams." },
  { name: "Ethan",  school: "Stanford'28",    photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Ethan&backgroundColor=d5f0f0",  quote: "With my counselor's guidance, I took my engineering projects, environmental leadership, and artistic portfolio and turned them into one standout application." },
  ...
  { name: "Sondre", school: "RISD'28",        photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Sondre&backgroundColor=d5e8f5", quote: "I've always had a passion for animation and art. With AddedEducation's guidance, I built a strong foundation and navigated the application process seamlessly." },
]
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `https://api-na2.hsforms.com/submissions/v3/integration/submit/4257853/89b9163d-9966-482f-b251-a601fc461802`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            context: {
              ...
              pageName: "AddedEducation — Contact Us",
            },
          }),
        }
      )
      if (res.ok) {
        setSubmitted(true)
        // If this person came from /diagnostic-test, their quiz result was
        // stashed (not saved yet) — pick it up now and save it in the SAME
        // row as this contact submission: one sequence, one row, identity
        // from this form.
        let pendingDiagnostic = null
        try {
          const raw = sessionStorage.getItem("ae_diagnostic_pending")
          if (raw) pendingDiagnostic = JSON.parse(raw)
        } catch { /* noop */ }

        // Also save into Added Admin (Diagnostic Test table) —
        // best-effort, never blocks the person's submitted confirmation.
        fetchData("diagnostic-test/submit-contact", {
          firstname: form.firstname.trim(),
          lastname: form.lastname.trim(),
          email: form.email.trim(),
          phone: fullPhone,
          school: form.schoolname.trim(),
          country: COUNTRIES.find(c => c.iso === form.countryIso)?.name || "",
        ...
        if (typeof window !== "undefined") {
          if (window.fbq) window.fbq("trackCustom", "Finally_Leads", { landing_page: "contact-us" })
          if (window.gtag) window.gtag("event", "Finally_Leads", { landing_page: "contact-us" })
        }
      } else {
