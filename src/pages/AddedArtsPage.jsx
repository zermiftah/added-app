...
const PROGRAMMES = [
  {
    num: "01", name: "Arts & Portfolio Consultancy",
  ...
  {
    num: "03", name: "The Upskilling Workshop",
  ...
  {
    num: "04", name: "The Portfolio Bootcamp",
...
const MENTORS = [
  {
    ...
    photo: "https://addededucation-assets.s3.us-east-1.amazonaws.com/hosts/gayatri.jpg",
  },
  {
    ...
    photo: "https://addededucation-assets.s3.us-east-1.amazonaws.com/hosts/linda.jpg",
  },
]

const SCHOOL_LOGOS = ["RISD", "SCAD", "Parsons", "Pratt", "SAIC", "UAL"]
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {imgOk
          ? <img src={m.photo} alt={m.name} onError={() => setImgOk(false)} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 10%", position: "absolute", inset: 0 }} />
...
@media(max-width:700px){.aa-result-row{grid-template-columns:1fr;gap:10px;}}
      ...
      <section style={{ background: C.ink, padding: "120px 0 80px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, background: `linear-gradient(150deg, ${C.maroonSoft} 0%, ${C.ink} 60%)`, pointerEvents: "none" }} />
          ...
          <Reveal delay={250}>
            <div style={{ marginTop: 64, display: "flex", flexWrap: "wrap", gap: 32, paddingTop: 18, borderTop: `1px solid ${C.borderLight}`, alignItems: "center" }}>
              {SCHOOL_LOGOS.map(name => (
                <span key={name} style={{ fontFamily: "'Fraunces',serif", fontStyle: "italic", fontSize: 17, color: "rgba(255,255,255,0.4)" }}>{name}</span>
              ))}
            </div>
          </Reveal>
        </div>
            ...
            {PROGRAMMES.map((p, i) => (
              <Reveal key={p.num} delay={i * 60}>
                <div className="aa-card" style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", height: "100%", display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ width: "100%", aspectRatio: "4/3", padding: 8, background: C.cream, flexShrink: 0, position: "relative" }}>
                    <div style={{ width: "100%", height: "100%", border: `1px solid ${C.border}`, background: `linear-gradient(155deg, ${C.creamWarm} 0%, ${C.stoneLight}30 100%)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, letterSpacing: 1, textTransform: "uppercase", color: C.stoneLight }}>Fig. {p.num}</span>
                    </div>
                  </div>
                  <div style={{ padding: "24px 24px 28px", display: "flex", flexDirection: "column", flex: 1 }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, fontWeight: 600, color: C.accent }}>{p.num}</span>
                ...
                <form onSubmit={handleSubmit}>
                  <div className="aa-form-grid">
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <select className={`aa-form-input${errors.country ? " aa-form-input-err" : ""}`} value={form.country} onChange={e => setForm({ ...form, country: e.target.value })}>
